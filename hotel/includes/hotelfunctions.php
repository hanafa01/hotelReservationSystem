...
function getHotels($gid = 0)
{
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        // get photos
        $query3 = "SElECT * FROM photos WHERE hotel_id='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["form"] == "signup") {
    ...
    $name = $_POST["name"];
    $phone = $_POST["phone"];
    $query = "INSERT INTO users(name,phone,username,password) VALUES('$name', '$phone','$username','$password')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["form"] == "cancel") {
    ...
    $connection->query($query);
    header("Location: {$GLOBALS['url']}profile.php");
...
function getuser($gid)
{
    ...
    $user = array();
    while ($row = mysqli_fetch_array($result)) {
        $query2 = "SElECT * FROM booking WHERE user_id='$gid' AND status='confirmed'";
        $result2 =$connection->query($query2);
