...
//
// Add new image
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["form"] == "galleryadd") {
    global $connection;
    $image = $_FILES['img']['name'];
    $target = "../uploads/".basename($image);
    $hotel_id = $_SESSION["hotel_id"];
    $query = "INSERT INTO gallery(hotel_id,src) VALUES('$hotel_id', '$image')";
...
function getGallery()
{
    global $connection;
    $hotel_id = $_SESSION["hotel_id"];
    $query = "SELECT * FROM gallery WHERE hotel_id='$hotel_id'";
    $result = $connection->query($query);
    $types = array();
    while ($row = mysqli_fetch_array($result)) {
        $types[] = $row;
...
// Delete image
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["form"] == "gallerydelete") {
    global $connection;
    $gallery_id = $_POST["gallery_id"];
    $hotel_id = $_SESSION["hotel_id"];
    $query = "DELETE FROM gallery WHERE id='$gallery_id' AND hotel_id='$hotel_id'";
    if ($connection->query($query)) {
        header("Location: {$GLOBALS['url']}admin/photos.php");
    } else {

        echo mysqli_error($connection);
    //   header("Location: {$GLOBALS['url']}admin/galleryshow.php");
    }
}
